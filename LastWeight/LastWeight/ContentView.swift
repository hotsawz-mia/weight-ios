                        ...
                        if let currentSample = healthKitManager.weightData.first {
                            ...
                            if let referenceWeight = lastClosestWeight {
                                ...
                                if let match = previousLowerMatch {
                                    ...
                                    let convertedDiff = weightUnit.convert(diff)
                                    Text("Lose \(String(format: "%.1f", convertedDiff)) \(weightUnit.rawValue) to match your weight from \(formatDateWithDaysAgo(match.date))")
                                        .font(.subheadline)
